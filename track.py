    ...
    for frame_idx, (path, im, im0s, vid_cap, s) in enumerate(dataset):
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if det is not None and len(det):
                ...
                det[:, :4] = scale_coords(
                    im.shape[2:], det[:, :4], im0.shape).round()  # xyxy
            ...
            if save_vid:
                if vid_path[i] != save_path:  # new video
                    ...
                    if vid_cap:  # video
                        ...
                        fps, w, h = 30, add_image.shape[1], add_image.shape[0]
                    # force *.mp4 suffix on results videos
                    save_path = str(Path(save_path).with_suffix('.mp4'))
                    vid_writer[i] = cv2.VideoWriter(
                        save_path, cv2.VideoWriter_fourcc(*'mp4v'), fps, (w, h))
                # vid_writer[i].write(im0)
                vid_writer[i].write(add_image)
...
def parse_opt():
    ...
    parser.add_argument('--tracking-method', type=str,
                        default='strongsort', help='strongsort, ocsort, bytetrack')
