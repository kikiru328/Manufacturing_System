...
# order_data_count = order_data['Count'].to_list()
# order_data_option = order_data['Option'].to_list()
...
if str(ROOT / 'trackers' / 'strong_sort' / 'deep' / 'reid' / 'torchreid') not in sys.path:
    sys.path.append(str(ROOT / 'trackers' / 'strong_sort' / 'deep' /
...
def run(
        ...
        appearance_descriptor_weights=WEIGHTS / \
    ...
    for frame_idx, (path, im, im0s, vid_cap, s) in enumerate(dataset):
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if show_vid:
                ...
                draw.text(order_org, order_count_text,
                          font=font, fill=(0, 255, 0))
                img = np.array(img)

                # cv2.putText(imb, order_count_text, order_org, font, fontScale, color, thickness, cv2.LINE_AA)

                # cv2.imshow(str(p), im0)
                # cv2.imshow(str(p), imb)
                # cv2.imshow(str(p), img)
                IMG = cv2.addWeighted(im0, alpha, img, (1-alpha), 0)
...
def parse_opt():
    ...
    parser.add_argument('--appearance-descriptor-weights',
                        type=Path, default=WEIGHTS / 'osnet_x0_25_msmt17.pt')
    ...
    parser.add_argument('--conf-thres', type=float,
                        default=0.7, help='confidence threshold')
...
def count_obj(box, w, h, id):
    ...
    # print('BOX : ', box)
    # if int(box[1]+(box[3] - box[1])/2) > (int(w/2)) & id not in data:
